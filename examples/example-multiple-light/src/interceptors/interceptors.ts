 ...
 */
export const forwardSideFx = (fxID: string): InterceptorFn => (_, [__, body]) => ({
 ...
 * @param event
 */
export const dispatch = (event: Event): InterceptorFn => () => ({ [FX_DISPATCH]: event })
 ...
 * @param event
 */
export const dispatchNow = (event: Event): InterceptorFn => () => ({ [FX_DISPATCH_NOW]: event })
 ...
 * @param id
 */
export const snapshot = (id = 'history'): InterceptorFn => (_, __, ___, ctx) => ctx[id].record()
 ...
 */
export const ensurePred = (pred: InterceptorPredicate, err?: InterceptorFn): InterceptorFn => (
  state,
  e,
  bus,
) =>
 ...
 */
export const ensureStateLessThan = (max: number, path?: (e: Event) => Path, err?: InterceptorFn) =>
 ...
 * @param fn
 */
export const valueUpdater = <T>(path: Path, fn: (x: T, ...args: any[]) => T): InterceptorFn => {
